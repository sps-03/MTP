...
unsigned long long TC_CPU(int numVertices, std::vector<std::vector<int>> &adjList, bool isDirected);
void checkCudaError();
...
int main(int argc, char **argv) {
    ...
    // to store the input graph in adjacency list format
    ...
    unsigned long long cpuTC = TC_CPU(numVertices, adjList, isDirected);

    // check for counts
    if(gpuTC != cpuTC) {
        printf("Initial Graph: Difference in CPU & GPU counts.!!!\ngTC=%llu cTC=%llu\n\n", gpuTC, cpuTC);
        return 0;
    }

    printf("Initial graph: gTC=%llu cTC=%llu\n", gpuTC, cpuTC);
    ...
    for(int i=0; i < numUpdates; i+=batchSize) {
        for(int j=0; j<batchSize; j++) {
            ...
            updates[j] = {type, u, v};
            if(type=='a') adjList[u].push_back(v);
            if(type=='d') adjList[u].erase(std::find(adjList[u].begin(), adjList[u].end(), v));

            if(!isDirected) {
                updates[++j] = {type, v, u};
                if(type=='a') adjList[v].push_back(u);
                if(type=='d') adjList[v].erase(std::find(adjList[v].begin(), adjList[v].end(), u));
            }
        }
    ...
    // final checking
    cpuTC = TC_CPU(numVertices, adjList, isDirected);
    
    // check for counts
    if(gpuTC != cpuTC) {
        printf("Final Graph: Difference in CPU & GPU counts.!!!\ngTC=%llu cTC=%llu\n\n", gpuTC, cpuTC);
        return 0;
    }

    printf("Final graph: gTC=%llu cTC=%llu\n\n", gpuTC, cpuTC);
